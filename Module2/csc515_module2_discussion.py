...
imgPath = r"C:\Projs\Python\opencv2\images\mukul"

# Face Mesh Detection
# mp_face_mesh = mp.solutions.face_mesh    # error, python version mismatch
# mp_drawing = mp.solutions.drawing_utils  # error, python version mismatch
# drawing_spec = mp_drawing.DrawingSpec(thickness=1, circle_radius=1)  # error, python version mismatch

def drawFaceMeshes(srcPath: str):  #ok
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for file in os.listdir(srcPath):
            ...
            if displayCount < 10:
                start = time.time()
                img = cv2.imread(f"{srcPath}/{file}")
                image = cv2.cvtColor(cv2.flip(img, 1), cv2.COLOR_BGR2RGB)
                image.flags.writeable = False
                results = face_mesh.process(image)                
                image.flags.writeable = True
                image = cv2.cvtColor(image, cv1.COLOR_RGB2BGR)
...
def translateImage2(imgFile: str, lx: int, ly: int, angle: float):
    if imgFile is None or len(imgFile.strip()) < 1:
    ...
    cv2.imshow("imgRightBill", imgRightBill)
    ...
    cv2.imshow("imgRotatedRightBill", imgRotatedRightBill)
...
def detect_fake_usd100Bill2(imgFile: str):
    if imgFile is None or len(imgFile.strip()) < 1:
        print(f"Invalid image path/file: {imgFile}")
        return
    imgFile = imgFile.strip()
    ...
    img = cv2.imread(imgFile)
    #img = cv2.imread("Photos/city 1.jpg")
    #img = cv2.imread("Photos/BMW335i-1.png")
    #img = cv2.imread("Photos/IMG_0002.JPG")
    h, w = img.shape[:2]
    new_width = 800
    new_height = int(h * (new_width / w))
    imgResized = cv2.resize(img, (new_width, new_height))
    cv2.imshow("$100", imgResized)

    imgGrayed = cv2.cvtColor( imgResized, cv2.COLOR_BGR2GRAY )
    cv2.imshow("$100 - Gray Scaled", imgGrayed)

    imgBlur = cv2.GaussianBlur(imgGrayed, (5,5), 0)
    cv2.imshow("$100 - Gray Blur", imgBlur)
    imgEqualized = cv2.equalizeHist(imgBlur)
    cv2.imshow("$100 - Gray Blur Equalized", imgEqualized)
    #canny2 = cv2.Canny(imgEqualized, 150, 175)  # highlights the outlines of objects
    canny2 = cv2.Canny(imgEqualized, 125, 200)  # highlights the outlines of objects
    cv2.imshow("$100 - Gray Blur Equalized Canny", canny2)

    contours, hierarchy = cv2.findContours( canny2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE )
    contour_img = imgResized.copy()
    ...
    cv2.imshow("$100 - Gray Scaled. Laplace", lap)
    ...
    canny = cv2.Canny(imgGrayed, 150, 175)
